...
def main():
	...
	input_path = inputs.get('input_ifc').get('components')[0]['path']
	project_id = inputs.get('input_ifc').get('project')
	mission_id = inputs.get('input_ifc').get('mission')
	offsets = inputs.get('input_ifc').get('offset')
	hsrs = inputs.get('input_ifc').get('horizontal_srs_wkt')
	vsrs = inputs.get('input_ifc').get('vertical_srs_wkt')
	...
	ifc2mesh(input_path, str(WORKING_DIR)+'/', offsets, hsrs, vsrs)


	logging.debug('Generating outputs.json file...')

	outpath = WORKING_DIR / 'output.glb'
	output = {
		"outputs": {
